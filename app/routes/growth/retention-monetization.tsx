...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[13px] font-semibold uppercase tracking-wider ${
...
function WinnerBadge({ winner }: { winner: string | null }) {
  if (!winner) return <span className="text-[15px] text-muted-foreground">Pending</span>;
  return (
    <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[13px] font-semibold bg-success/10 text-success">
    ...
    <div className="space-y-8">
      ...
      <div className="animate-in">
        <div className="flex items-center gap-2 mb-1">
          <span className="text-[15px] font-semibold text-primary uppercase tracking-wider">Tier 1 — Foundation</span>
        </div>
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-border">
                <th className="text-left text-[13px] font-semibold text-muted-foreground uppercase tracking-wider pb-3 pr-4">Cohort</th>
                <th className="text-right text-[13px] font-semibold text-muted-foreground uppercase tracking-wider pb-3 px-4">Customers</th>
                <th className="text-right text-[13px] font-semibold text-muted-foreground uppercase tracking-wider pb-3 px-4">MRR</th>
                <th className="text-right text-[13px] font-semibold text-muted-foreground uppercase tracking-wider pb-3 px-4">Retention</th>
                <th className="text-right text-[13px] font-semibold text-muted-foreground uppercase tracking-wider pb-3 pl-4">Churn</th>
              </tr>
            </thead>
            <tbody>
              {cohorts.map((cohort) => (
                <tr key={cohort.cohort} className="border-b border-border/50">
                  ...
                  <td className="py-2.5 px-4 text-base font-mono text-right text-secondary-foreground">{cohort.customers}</td>
                  <td className="py-2.5 px-4 text-base font-mono text-right text-foreground">${cohort.mrr.toLocaleString()}</td>
            ...
            <div key={experiment.id} className="card">
              <div className="flex items-start justify-between mb-4">
                <div className="flex-1 min-w-0">
                  ...
                  {experiment.hypothesis && (
                    <p className="text-base text-secondary-foreground leading-relaxed">{experiment.hypothesis}</p>
                  )}
                </div>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                <div className={`p-4 rounded-lg border ${experiment.winner === "A" ? "border-success/30 bg-success/5" : "border-border bg-card"}`}>
                  <div className="flex items-center gap-2 mb-2">
                    <span className="text-[13px] font-semibold text-muted-foreground uppercase tracking-wider">Variant A</span>
                    {experiment.winner === "A" && <CheckCircle2 className="w-3.5 h-3.5 text-success" />}
                  </div>
                  <p className="text-base font-medium text-foreground">{experiment.variant_a.plan}</p>
                  <p className="text-2xl font-mono font-semibold text-foreground mt-1">
                    ${experiment.variant_a.price}
                    <span className="text-[15px] text-muted-foreground font-normal">/{experiment.variant_a.billing}</span>
                  </p>
                </div>
                <div className={`p-4 rounded-lg border ${experiment.winner === "B" ? "border-success/30 bg-success/5" : "border-border bg-card"}`}>
                  <div className="flex items-center gap-2 mb-2">
                    <span className="text-[13px] font-semibold text-muted-foreground uppercase tracking-wider">Variant B</span>
                    {experiment.winner === "B" && <CheckCircle2 className="w-3.5 h-3.5 text-success" />}
                  </div>
                  <p className="text-base font-medium text-foreground">{experiment.variant_b.plan}</p>
                  <p className="text-2xl font-mono font-semibold text-foreground mt-1">
                    ${experiment.variant_b.price}
                    <span className="text-[15px] text-muted-foreground font-normal">/{experiment.variant_b.billing}</span>
                  </p>
                </div>
              </div>
              <div className="flex items-center gap-6 pt-4 border-t border-border">
                {experiment.start_date && (
                  <div>
                    <p className="text-[13px] text-muted-foreground uppercase tracking-wider">Started</p>
                    <p className="text-base font-mono text-secondary-foreground">
                ...
                {experiment.revenue_impact != null && (
                  <div>
                    <p className="text-[13px] text-muted-foreground uppercase tracking-wider">Revenue Impact</p>
                    <p className={`text-base font-mono font-semibold ${experiment.revenue_impact >= 0 ? "text-success" : "text-danger"}`}>
                ...
                {experiment.statistical_significance != null && (
                  <div>
                    <p className="text-[13px] text-muted-foreground uppercase tracking-wider">Significance</p>
                    <p className={`text-base font-mono font-semibold ${experiment.statistical_significance >= 95 ? "text-success" : "text-warning"}`}>
        ...
        <div className="card animate-in stagger-6">
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead>
                <tr className="border-b border-border">
                  <th className="text-left text-[13px] font-semibold text-muted-foreground uppercase tracking-wider pb-3 pr-4">Feature</th>
                  <th className="text-center text-[13px] font-semibold text-muted-foreground uppercase tracking-wider pb-3 px-4">Status</th>
                  <th className="text-right text-[13px] font-semibold text-muted-foreground uppercase tracking-wider pb-3 px-4">Adoption</th>
                  <th className="text-right text-[13px] font-semibold text-muted-foreground uppercase tracking-wider pb-3 pl-4">Retention Impact</th>
                </tr>
              </thead>
              <tbody>
                {retentionFeatures.map((feature) => (
                  <tr key={feature.id} className="border-b border-border/50">
                    <td className="py-2.5 pr-4">
                      <p className="text-base font-medium text-foreground">{feature.feature_name}</p>
                    </td>
